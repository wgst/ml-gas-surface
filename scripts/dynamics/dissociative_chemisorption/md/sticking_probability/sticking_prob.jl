...
@everywhere begin
    ...
    function ensemble_processing(ensemble, dist_cutoff, scat_cutoff, atoms, cell, model_h2, surface, e_tran, cur_folder, n_atoms_layer)
        ...
        # sim_h2 = Simulation{Classical}(Atoms([:H,:H]), model_h2, cell=cell)
    ...
    function (out::OutputTrajectory)(sol, i)
        if out.save_errors == true
            db_out = db.connect("$(out.path)output_h2$(out.surface)_E$(out.e_tran)_traj.db")
    ...
    icond_f = "$(ENV["H2CU_ML_INIT_COND"])/output/$(model_folder)_8x8/"
    ...
    save_errors = false # choose if you want to save structures with min error of v_models_error for adaptive sampling 
    ...
    ase_atoms = io.read("$(icond_str)$(surface)_h$(height)_full_$(temp_surf)K_8x8_ace.in")
    cell = system["cell"]
...
@time ensemble = Ensembles.run_dynamics(sim, (0.0, max_time), distribution; selection=traj_start:traj_end, dt=step, trajectories=traj_num, 
                        output=OutputTrajectory(atoms, cell, cur_folder, surface, string(e_tran), models_as_schnet, traj_start, v_models_error, save_errors),
