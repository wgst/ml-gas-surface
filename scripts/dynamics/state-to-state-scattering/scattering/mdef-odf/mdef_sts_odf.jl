...
@everywhere begin
    using Pkg
    Pkg.activate("/home/m/msrmnk2/FrictionProvidersODF_aceds/cur")
    Pkg.instantiate()
end
...
@everywhere begin
    ...
    # LOAD EFT ODF MODEL
    eft_model_ace = read_dict(load_dict(eft_model_path)) # load eft model
    friction_ids = [length(ase_atoms)-1,length(ase_atoms)]
    aceds_model = ACEdsODF(eft_model_ace, Gamma, atoms_julip; friction_unit=u"ps^-1")
    eft_model = ODFriction(aceds_model; friction_atoms=friction_ids)
